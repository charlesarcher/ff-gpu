...
/* The following calculates integer pairs such that all of Sammy's and Peter's above statements are true. */
  ...
  #define THREADS 31
#endif
// Number of sums per thread. Only makes a difference for sums whose prime calculations generally exceed the prime memory. With these defaults that's about 5000000.
#ifndef NUMSUM
  #define NUMSUM 5000
#endif
...
// Free AI special #1. I modified it a lot.
double getCpuSpeedMhz()
 {ifstream file("/proc/cpuinfo");
  ...
  file.close();
  return coreCount>0 ? totalMhz/coreCount : 0.0;
 }




// Free AI special #2. Seems ya always must screw with it.
// Hate that sudo prompt when getting memory size? Put 'username ALL=(ALL:ALL) NOPASSWD: /usr/sbin/dmidecode' in file /etc/sudoers.d/username.
string getMemorySpeed()
 {array<char, 128> buffer;
  string result;
  // Query DMI type 17 for memory device attributes
  unique_ptr<FILE,int32_t(*)(FILE*)>pipe(popen("sudo dmidecode --type 17 2>/dev/null | grep -i 'Speed:'", "r"), pclose);
  if (!pipe) return "Popen failed";
  result="0 MT/s";
  if (fgets(buffer.data(), buffer.size(), pipe.get())!=nullptr) 
   {result=buffer.data(); // Note buffer.data() has a trailing new line character.
    uint64_t first=result.find_first_not_of(" \t\n\r\f\v");
...
int main (int argc, char* argv[])
 {uint32_t sumStart,
  ...
  auto pTime = duration_cast<microseconds>(high_resolution_clock::now()-start);

  start = high_resolution_clock::now();
  auto fpmTime = high_resolution_clock::duration::zero(),
       fnpmTime = high_resolution_clock::duration::zero();
  uint32_t pmapSumLimit=isqrt64((prime.MaxPrimeMapValue()-2)<<2)<<1;
  FreudenthalThreads freudenthalThreads;
  // This is for load balancing after threads start using Miller-Rabin more than the prime map.
  if (sumLimit<=pmapSumLimit)
   ...
   fpmTime = high_resolution_clock::now()-start;
   }
  else
   {uint32_t subSumLimit=NUMSUM*threads, // NUMSUM is a subjective choice. This is the # of sums to process before renormalization. What works for you?
             subSumStart=pmapSumLimit+1|1,
             subSumDelta=sumLimit-subSumStart+1,
             subSumDivisor=max(subSumDelta,subSumLimit)/subSumLimit;
    double subSumDecayRate=double(subSumDivisor-1)/subSumDivisor;
    uint32_t subSumSize=min(uint32_t(subSumLimit/(1-pow(subSumDecayRate,double(subSumDivisor)))),subSumDelta);
    cout<<"Start Segment Size: "<<subSumSize<<",  Target Segment Size: "<<subSumLimit<<",  Number of Segments: "<<subSumDivisor<<",  DecayRate: "<<subSumDecayRate<<endl;
    freudenthalThreads.GenerateThreads(sumStart, pmapSumLimit, productLimit, prime); // Send some Freudenthal values to stdout
    fpmTime = high_resolution_clock::now()-start;
    auto fnpmStart = high_resolution_clock::now();  
    if (subSumDecayRate)
      for (uint32_t segment=0; segment<subSumDivisor; ++segment)
       {uint32_t subSumEnd=subSumStart+subSumSize;
        freudenthalThreads.GenerateThreads(subSumStart, subSumEnd, productLimit, prime); // Rernormalize and send more Freudenthal values to stdout
        subSumStart=subSumEnd+1|1;
        subSumSize*=subSumDecayRate;
       }
    if (subSumStart<=sumLimit)
      freudenthalThreads.GenerateThreads(subSumStart, sumLimit, productLimit, prime); // Rernormalize and send any remaining Freudenthal values to stdout
    fnpmTime=high_resolution_clock::now()-fnpmStart;
   }
  auto fTime = duration_cast<microseconds>(high_resolution_clock::now()-start);
