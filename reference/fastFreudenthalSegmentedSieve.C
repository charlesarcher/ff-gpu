...
// Hate that sudo prompt when getting memory size? Issue 'sudo chmod 444 /sys/firmware/dmi/tables/*' at a command line prompt.
 ...
 {uint32_t sumStart,
  ...
  if (sumLimit<=pmapSumLimit)
   ...
   {freudenthalThreads.GenerateThreads(sumStart, pmapSumLimit, productLimit, prime); // Send some Freudenthal values to stdout
    ...
    int32_t subSumLimit=NUMSUM*threads; // NUMSUM is a subjective choice. This is the # of sums to process before renormalization. What works for you?
    uint32_t subSumStart=pmapSumLimit+1|1,
             subSumDelta=sumLimit-subSumStart+1,
             subSumDivisor=max(subSumDelta,uint32_t(subSumLimit))/subSumLimit;
    int32_t subSumSize=min(int32_t(subSumLimit*pow(double(subSumDivisor)/double(subSumDivisor-1),double(subSumDivisor-1))),int32_t(subSumDelta));
    while (subSumSize>subSumLimit)
     {uint32_t subSumEnd=subSumStart+subSumSize;
      ...
      subSumSize=(subSumSize*(subSumDivisor-1))/subSumDivisor;
     }
