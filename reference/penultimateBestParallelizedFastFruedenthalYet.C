...
Prime::Prime(const uint64_t limit, const uint32_t nPrimes): numPrimes(0)
 {uint64_t numPrimesRequested=nPrimes>1 ? nPrimes : 1;// numPrimesRequested is number of array addressable primes.
  ...
  atomic_uint64_t blockVal[pthreads]={};
  atomic_bool threadActive[pthreads]={};
  Stage2ThreadData s2Data[pthreads];
  thread threadList[pthreads];
  uint32_t curList=0;
  for (uint64_t p=3; p<=sqrtLimit; p+=2)
   {uint32_t prevList=(curList ? curList : pthreads)-1;
    while (prevList!=curList)
     {if (p>blockVal[prevList]&&threadList[prevList].joinable()) threadList[prevList].join(); // Wait on any previous threads that need to finish before the prime map can be trusted
      prevList=(prevList ? prevList : pthreads)-1;
     }
    if (p>blockVal[curList]&&threadList[curList].joinable()) threadList[curList].join(); // Wait on current thread that may need to finish before the prime map can be trusted
      
    atomic_uchar* ap=(atomic_uchar*)&primeMap[p>>4];
    if (*ap&0x80>>(p>>1&7))
     {if (numPrimes<numPrimesRequested) primes[numPrimes++]=p;
      // Find a thread that is done. This is a polling operation; it goes until one is found potentially locking this thread in a tight loop if a needed thread never frees up.
      //   One should always free up...just sayin'.
      for (; threadActive[curList]; curList=++curList<pthreads ? curList : 0);
      if (threadList[curList].joinable()) threadList[curList].join(); // Make sure the previous thread gets all cleaned up or things just up and barf.
      blockVal[curList]=p*p;
      threadActive[curList]=True;            
      threadList[curList++]=thread(&PrimeWheelStage1, p, primeMap, maxPrimeMapValue, &blockVal[curList], &threadActive[curList], &s2Data[curList]);
      if (curList>=pthreads) curList=0;
     }
   }
  for (uint32_t i=0; i<sizeof(threadList)/sizeof(thread); ++i) 
   {if (threadList[i].joinable()) threadList[i].join(); // Wait for all stage 1 threads
    if (s2Data[i].s2Thread.joinable()) s2Data[i].s2Thread.join(); // Wait for all stage 2 threads
   }
...
class FreudenthalThreads
 {typedef int Boolean;
  ...
  public:
    FreudenthalThreads(const uint32_t sumStart, const uint32_t sumLimit, const uint64_t productLimit, const Prime& prime);
...
FreudenthalThreads::FreudenthalThreads(const uint32_t sumStart, const uint32_t sumLimit, const uint64_t productLimit, const Prime& prime)
 {// Kick off threads
...
int main (int argc, char* argv[])
 {uint32_t sumStart,
  ...
  uint64_t pmapSumLimit=((isqrt64((prime.MaxPrimeMapValue()-2)<<2)<<1)-1)<<1;
  if (sumLimit<=pmapSumLimit)
    FreudenthalThreads(sumStart, sumLimit, productLimit, prime); // Send Freudenthal values to stdout
  else
   {FreudenthalThreads(sumStart, pmapSumLimit, productLimit, prime); // Send some Freudenthal values to stdout
    FreudenthalThreads(pmapSumLimit+1|1, sumLimit, productLimit, prime); // Rernormalize and send remaining Freudenthal values to stdout
   }
