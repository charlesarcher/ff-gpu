...
// including the vendor TU (src/hip_enum.cpp) which fills it through the
// extern "C" entry point.
//
// POD only (fixed-size char arrays + plain integral fields, no std:: types) so
// the object layout is identical whether a TU is compiled by g++ or the HIP
// clang frontend — the struct crosses the vendor-TU boundary by pointer,
// never by value.
...
struct DeviceInfo {
    ...
    char name[256];          // runtime-reported device name (hipDeviceProp_t .name)
    ...
    unsigned long long freeBytes = 0;   // free VRAM at init (hipMemGetInfo)
    unsigned long long totalBytes = 0;  // total VRAM (hipMemGetInfo)
